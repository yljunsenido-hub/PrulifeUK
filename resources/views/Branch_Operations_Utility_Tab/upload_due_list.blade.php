...
<head>
  ...
  <script>
    // Function to get the first Monday and last day of the current month
    function getMonthlyDates() {
      ...
      const lastDayOfMonth = new Date(year, month + 1, 0); // 0 gets the last day of the previous month

      // Format the dates to YYYY-MM-DD
      const formatDate = (date) => date.toISOString().split('T')[0];

      // Set the dates on the page
      document.getElementById('start-date').textContent = formatDate(firstMonday);
      document.getElementById('end-date').textContent = formatDate(lastDayOfMonth);
    }
